...
import { notebookSources } from './data.js';
import './auth.js';
...
async function loadData() {
    ...
    // Fetch painpoints
    const { data: pps } = await supabase.from('painpoints').select('*');
    if (pps && pps.length > 0) {
...
async function syncDefaultsToSupabase(userId) {
    ...
    for (const pp of defaults.painpoints) {
        ...
        await supabase.from('painpoints').upsert({ ...ppData, user_id: userId });
        for (const q of questions) {
            const { answers, ...qData } = q;
            await supabase.from('questions').upsert({ ...qData, painpoint_id: pp.id, user_id: userId });
        }
    }
    for (const doc of defaults.documents) {
        await supabase.from('documents').upsert({ ...doc, user_id: userId });
    }
}
...
async function saveLawyer(lawyer) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const { data, error } = await supabase.from('lawyers').upsert({ ...lawyer, user_id: user.id }).select();
...
async function saveAnswer(answer) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const { data } = await supabase.from('answers').upsert({ ...answer, user_id: user.id }).select();
    return data ? data[0] : null;
}
...
async function saveDocument(doc) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const { data } = await supabase.from('documents').upsert({ ...doc, user_id: user.id }).select();
    return data ? data[0] : null;
}
...
async function updatePainpointStatus(id, status) {
    await supabase.from('painpoints').update({ status }).eq('id', id);
}
        ...
        painpoints: [
            {
                ...
                short_desc: 'Gustavo wants to reset your vesting to zero (Feb 2026). You have been working since Jan 2024. He claims the Spanish amendment resets everything because of "new funding."',
            ...
            },
            {
                ...
                short_desc: 'The draft says if you are fired under Article 80 of the Saudi Labor Law, you are a "Bad Leaver" and lose your shares. Article 80 covers "disobedience" or "not following instructions" — totally subjective.',
            ...
            },
            {
                id: 'fakedebt',
                title: 'The "Fake Debt"',
                short_desc: 'Your notarized Articles of Association (AoA) say your capital is "Paid in Full." Gustavo\'s draft says you still owe it — creating a legal debt that doesn\'t exist.',
            ...
            },
            {
                ...
                short_desc: 'If you are a "Bad Leaver," the company buys your shares for "Nominal Value" (pennies). Since the company is worth millions (per the SAFE note), Gustavo can fire you and take your millions for 9,000 SAR.',
            ...
            },
            {
                ...
                short_desc: 'You only have 10 days to "fix" a breach. This is unreasonably short for a CTO dealing with complex technical or business issues.',
            ...
            },
            {
                ...
                short_desc: 'A recently signed amendment extended the "Cliff" (waiting period) for the Spanish entity, but is being used to justify a "Vesting Reset" (earning period) in the Saudi LLC.',
        ...
        documents: [
            {
                ...
                file_name: 'ContractByNationalNumber.pdf',
                description: 'Official Saudi government document proving 38.5% partnership and capital "Paid in Full."',
                key_section: 'Article 6 (Capital)',
                question_to_ask: 'Since this Ministry-stamped document says my capital is paid, is it legal for my partner to force me to sign a side-contract saying I still owe it?',
                shared_with: [],
                is_default: true
            },
            {
                ...
                file_name: 'Jaume Cases Marco - Postmoney Safe...pdf',
                ...
                key_section: 'Page 1 — Date: Sept 2024, $7M Valuation Cap',
                question_to_ask: 'This investment was made in 2024 based on my role as a founder. Does this prove that my Vesting Start Date should be January 17, 2024, regardless of when the Saudi LLC was registered?',
                shared_with: [],
                is_default: true
            },
            {
                ...
                file_name: 'Rommaana - amendment Dec 25.pdf',
                ...
                key_section: 'Clause 1.1 (Extension of Cliff Period)',
                question_to_ask: 'I signed this extension for a Spanish entity. Does a "Cliff Extension" legally allow my partner to "Reset" my vesting years to zero in the new Saudi company? Or should I still get credit for my 2024 start date?',
                shared_with: [],
                is_default: true
            },
            {
                ...
                file_name: 'Founders Agreement- Bilingual (Final).pdf',
                ...
                key_section: 'Page 36 — Bad Leaver Table',
                question_to_ask: 'Is it standard in KSA for a 38.5% Partner to be subject to Article 80 of the Labor Law as a reason to lose their equity? Shouldn\'t "Bad Leaver" be restricted to Objective Criminal Acts or a Final Court Judgment?',
                shared_with: [],
                is_default: true
            }
        ]
...
function renderDashboard() {
    ...
    state.painpoints.forEach((pp, i) => {
        ...
        <div class="card" data-ppid="${pp.id}">
          ...
          <div class="card-desc">${pp.short_desc}</div>
...
function renderPainpointDetail(ppId) {
    ...
    pp.questions.forEach(q => {
        ...
        if (q.answers.length === 0) {
            ...
            q.answers.forEach((a, idx) => {
                const lawyer = state.lawyers.find(l => l.id === a.lawyer_id);
    ...
    const relatedDocs = notebookSources.filter(d => d.painpointIds && d.painpointIds.includes(ppId));
...
function renderLawyers() {
    ...
    state.lawyers.forEach(l => {
        ...
        const totalAnswers = state.painpoints.reduce((sum, pp) =>
            sum + pp.questions.reduce((s, q) => s + q.answers.filter(a => a.lawyer_id === l.id).length, 0), 0);
        html += `
        <div class="lawyer-card">
          ...
          <div class="lawyer-meta">
            ...
            ${l.consult_date ? `<span>📅 ${l.consult_date}</span>` : ''}
...
function renderDocuments() {
    ...
    state.documents.forEach(doc => {
        let sharedHTML = `<div class="doc-shared"><span class="doc-shared-label">${t('shared_with')}</span>`;
        if (state.lawyers.length === 0) {
            sharedHTML += `<span style="font-size:0.72rem;color:var(--text-muted);font-style:italic;">${t('addLawyersFirst')}</span>`;
        } else {
            state.lawyers.forEach(l => {
                const isShared = doc.shared_with.includes(l.id);
                if (isShared) {
        ...
        <div class="doc-item">
          <div class="doc-header">
            ...
            <div>
              ...
              <div class="doc-filename">${escapeHTML(doc.file_name || '')}</div>
              ${doc.key_section ? `<div style="margin-top:4px;font-size:0.75rem;color:var(--accent-amber);">📌 ${t('key_section')}: ${escapeHTML(doc.key_section)}</div>` : ''}
            </div>
            <div style="margin-left:auto;display:flex;gap:6px;">
              <button class="btn-icon btn-sm" title="Edit" onclick="window._editDoc('${doc.id}')">✏️</button>
              ${!doc.is_default ? `<button class="btn-icon btn-sm" title="Delete" onclick="window._deleteDoc('${doc.id}')">🗑️</button>` : ''}
            </div>
          </div>
          <div class="doc-desc">${escapeHTML(doc.description || '')}</div>
          ${doc.question_to_ask ? `<div class="doc-question">${escapeHTML(doc.question_to_ask)}</div>` : ''}
          ${sharedHTML}
        </div>`;
    });
...
// ─── View Doc Modal ────────────────────
window._viewDoc = (id) => {
    const doc = notebookSources.find(d => d.id === id);
    if (!doc) return;
...
function renderCompareTable() {
    ...
    allQuestions.forEach(q => {
        ...
        state.lawyers.forEach(l => {
            const ans = q.answers.find(a => a.lawyer_id === l.id);
            if (ans) {
...
// ─── Lawyer CRUD ───────────────────────
function openLawyerModal(lawyer_id) {
    const form = $('#lawyerForm');
    form.reset();
    $('#lawyerEditId').value = '';

    if (lawyer_id) {
        const l = state.lawyers.find(x => x.id === lawyer_id);
        if (!l) return;
        ...
        $('#lawyerDate').value = l.consult_date || '';
...
// ─── Answer CRUD ───────────────────────
window._addAnswer = (question_id) => {
    if (state.lawyers.length === 0) {
        alert(t('alertAddLawyerFirst'));
        return;
    }
    $('#answerForm').reset();
    $('#answerQuestionId').value = question_id;
    $('#answerEditIdx').value = '';

    // Find question text
    let qText = '';
    state.painpoints.forEach(pp => {
        const q = pp.questions.find(q => q.id === question_id);
        if (q) qText = q.text;
    });
...
window._editAnswer = (question_id, ansIdx) => {
    let question = null;
    state.painpoints.forEach(pp => {
        const q = pp.questions.find(q => q.id === question_id);
        if (q) question = q;
    ...
    let opts = `<option value="">${t('selectLawyerPlaceholder')}</option>`;
    state.lawyers.forEach(l => { opts += `<option value="${l.id}" ${l.id === a.lawyer_id ? 'selected' : ''}>${l.name}</option>`; });
    $('#answerLawyer').innerHTML = opts;
...
window._deleteAnswer = (question_id, ansIdx) => {
...
$('#answerForm').addEventListener('submit', e => {
    e.preventDefault();
    const question_id = $('#answerQuestionId').value;
    const editIdx = $('#answerEditIdx').value;
    const lawyer_id = $('#answerLawyer').value;
    const text = $('#answerText').value.trim();
    const recommendation = $('#answerRecommendation').value;

    if (!lawyer_id) { alert(t('alertSelectLawyer')); return; }
...
function openDocModal(docId) {
    ...
    if (docId) {
        ...
        $('#docFileName').value = d.file_name || '';
        $('#docDesc').value = d.description || '';
        $('#docSection').value = d.key_section || '';
        $('#docQuestion').value = d.question_to_ask || '';
    } else {
...
$('#docForm').addEventListener('submit', e => {
    ...
    const data = {
        ...
        file_name: $('#docFileName').value.trim(),
        description: $('#docDesc').value.trim(),
        key_section: $('#docSection').value.trim(),
        question_to_ask: $('#docQuestion').value.trim()
    };
...
window._toggleDocShare = (docId, lawyer_id) => {
    const doc = state.documents.find(d => d.id === docId);
    if (!doc) return;
    const idx = doc.shared_with.indexOf(lawyer_id);
    if (idx >= 0) {
        doc.shared_with.splice(idx, 1);
    } else {
        doc.shared_with.push(lawyer_id);
    }
    saveDocument({ id: doc.id, shared_with: doc.shared_with }).then(() => {
        loadData();
    });
};
